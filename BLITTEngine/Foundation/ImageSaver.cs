...
namespace BLITTEngine.Foundation
{
    ...
    class IDatChunk : PngChunk
    {
        ...
        public async Task<byte[]> GetCompressedDataAsync()
        {
            ...
            for (int i = 0; i < imageInfo.Height; i++)
            {
                ...
                Buffer.BlockCopy(scanline, 0, scanlineToWrite, 1, scanline.Length);
    ...
    class TransparencyMap
    {
        ...
        public void AddTransparencyToMap(int palletteIndex, byte alpha)
        {
    ...
    class PngFile : IEnumerable<PngChunk>
    {
        readonly List<PngChunk> chunksToWrite = new List<PngChunk>();
        static readonly byte[] Magic = { 0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a };

        public void Add(PngChunk chunk) => chunksToWrite.Add(chunk);

        IEnumerator<PngChunk> IEnumerable<PngChunk>.GetEnumerator() => chunksToWrite.GetEnumerator();
    ...
    {
        public static async void Save(byte[] pixels, int width, int height, string filePath)
        {
            try
            {
                var header = new IHdrChunk(width, height, 8, ColorType.TruecolorAlpha);
                var idat = new IDatChunk(header, pixels);
            ...
            {
                Game.ThrowError("Error saving Pixmap to file: {0}", e.Message);
            }
